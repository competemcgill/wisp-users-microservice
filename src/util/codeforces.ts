...
export const codeforces = {
    updateUserProblems: async (user: IUserModel) => {
        ...
        submissions.some((submission, i: number) => {
            const problemId: string = crypto
                .createHash("sha1")
                .update(
                    "CODEFORCES" +
                        submission.problem.contestId +
                        submission.problem.index.toUpperCase()
                )
                .digest("hex");
            ...
            if (
                problemId ==
                    user.platformData.codeforces.lastSubmission.problemId &&
                status == user.platformData.codeforces.lastSubmission.status
            )
                return true;
        ...
        for (const problem of problems) {
            try {
                const check = await axios.get(
                    `${process.env.WISP_PROBLEMS_URL}/problems/${problem.problemId}/exists`
                );
                if (check.status == 200) {
                    const problemIndex = user.problems.findIndex(
                        (dbProblem) => problem.problemId === dbProblem.problemId
                    );
